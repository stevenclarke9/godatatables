...
func TestMultilineColumnSelect(t *testing.T) {
	var stringTableWithoutHeader string = `1|aa|bb
2|"dd
ee
ff"|gg
3|cc|dd
`
	myVerbose = true
	stringTableReader := strings.NewReader(stringTableWithoutHeader)
	datatable, err := godatatables.ReadTable(stringTableReader, false)
	if err != nil {
		t.Errorf("error recorded %s", err)
	}
	sprintDataTableFromStringTableReader := fmt.Sprint(datatable)
	printOrLog(t, "sprintDataTableFromStringTableReader:")
	printOrLog(t, sprintDataTableFromStringTableReader)
	columnIndex := 1
	columnValue := `aa
...
`
	printOrLog(t, fmt.Sprint("column Index: ", columnIndex, " Value: ", columnValue,"\n"))
	selectDataTable := datatable.Select([]int{1})
